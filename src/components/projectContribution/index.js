...
import {
  Spin, PageHeader, Button, Tooltip, Table, Space, Typography, Select, Modal, DatePicker, Row, Col,
} from 'antd';
import {
...
const ProjectContribution = () => {
  ...
  const handleView = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_BASE_URL}/promTool/promReports/getProjectContribution`, body)
      .then((res) => {
        if (res.status === 200) {
          if (res.data.data.contributionArr.length !== 0) {
            if (state.textStateValues.startMonth !== null) {
              ...
              setState((prevState) => ({
                ...
                textStateValues: {
                  ...state.textStateValues,
                  modalsTitle: 'Project Contribution Report',
                  modalsIcon: infoIcon,
  ...
  const tableColumns = [
    ...
    {
      title: 'Contribution (in value based on PO/SOW)',
      dataIndex: 'contributionInvaluebasedonPOorSOW',
      key: 'contributionInvaluebasedonPOorSOW',
      width: 100,
      onCell: () => {
        return {
          style: { background: 'none' },
    ...
    {
      title: 'Contribution (in % based on PO/SOW)',
      dataIndex: 'contributionInPercentbasedonPOorSOW',
      key: 'contributionInPercentbasedonPOorSOW',
      width: 100,
      onCell: () => {
        return {
          style: { background: 'none' },
  ...
  const handleExportExcel = async () => {
    ...
    // eslint-disable-next-line max-len
    const fileName = `RM_Project_Contribution_Report_${moment().format('DD-MMM-YYYY')}_${new Date().getHours()}.${new Date().getMinutes()}.${new Date().getSeconds()}`; // Name to created/downloaded Excel file

    try {
      // creating worksheet1 in workbook
      const worksheet1 = workbook.addWorksheet('Project Contribution', {
      ...
      const projectContributionSummaryTable = [
        [],
        [' ', 'Project Contribution Report'],
        [],
        [' ', ...tableColumnHeaders],
      ];
      worksheet1.addRows(projectContributionSummaryTable);
        ...
        row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
          if (rowNumber === 2 && !(colNumber === 1)) {
            ...
            if (colNumber === 10) {
              ...
              if (cell.value <= 25) {
                color = 'FF0000';
              } else if (cell.value >= 26 && cell.value <= 35) {
                color = 'FFA500';
              } else if (cell.value >= 36 && cell.value <= 50) {
                color = 'FFFF00';
              } else {
                color = '008000';
              }
      ...
      setState((prevState) => ({
        ...
        textStateValues: {
          ...state.textStateValues,
          modalsTitle: 'Export Project Contribution Report',
          modalsIcon: failureIcon,
      ...
      workbook.removeWorksheet('Project Contribution');
    }
  };
            ...
            ? (
              ...
              <div>
                <div className="page-header-searchbar-flex">
                  <PageHeader>Project Contribution</PageHeader>
                  <Space>
                    <Tooltip placement="bottom" title="View Project Contribution">
                      <Button
                        type="primary"
                    ...
                    <Tooltip placement="bottom" title="Help">
                      <a href="/rm-tool/help/reports/contribution/project-contribution" target="_blank">
